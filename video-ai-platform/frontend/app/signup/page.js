    ...
    <div style={{ minHeight: '100vh', background: 'var(--bg)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '1.5rem', fontFamily: font }}>
      ...
      <div style={{ width: '100%', maxWidth: 420 }}>
        ...
        <div style={{ textAlign: 'center', marginBottom: '3rem' }}>
          ...
          <h1 style={{ fontFamily: font, fontWeight: 200, fontSize: '2.5rem', color: 'var(--on-surface)', letterSpacing: '0.04em', marginBottom: '0.5rem' }}>Create Account</h1>
          <p style={{ color: 'var(--on-muted)', fontSize: '0.875rem', fontWeight: 300, letterSpacing: '0.05em' }}>Start understanding videos with AI</p>
        </div>

        <form onSubmit={handleSignup} style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
          {message && (
            <div style={{ background: isSuccess ? 'rgba(52,211,153,0.05)' : 'rgba(238,125,119,0.06)', border: `1px solid ${isSuccess ? 'rgba(52,211,153,0.2)' : 'rgba(238,125,119,0.2)'}`, borderRadius: 4, padding: '0.75rem 1rem', color: isSuccess ? '#6ee7b7' : '#ee7d77', fontSize: '0.825rem', fontWeight: 300, display: 'flex', alignItems: 'center', gap: '0.5rem', fontFamily: font, boxShadow: `${isSuccess ? '0 0 15px rgba(52,211,153,0.1), inset 0 0 15px rgba(52,211,153,0.05)' : '0 0 15px rgba(238,125,119,0.1), inset 0 0 15px rgba(238,125,119,0.05)'}`, transition: 'all 0.3s ease' }}>
        ...
        <p style={{ textAlign: 'center', marginTop: '1.75rem', color: 'var(--on-muted)', fontSize: '0.85rem', fontWeight: 300, letterSpacing: '0.03em', fontFamily: font }}>
          ...
          <Link href="/login" style={{ color: 'var(--primary)', fontWeight: 300 }}>Sign in</Link>
        </p>
      </div>
